...
SUPPORTED_ACTIONS=("k8s_auth_aws" "k8s_auth_azure" "k8s_auth_gcp")
...
function refresh_token_k8s() {
  ...
  while read -r region; do
    ...
    run_or_print "terraform apply -refresh=false ${terraform_apply_args} -var-file=../../../${TFVARS_JSON} -var=${cloud_provider}_k8s_region=${region} -var=cluster_id=${index}"
