...
SUPPORTED_ACTIONS=("aws_k8s" "azure_k8s" "gcp_k8s"
...
function deploy_infra() {
  ...
  while read -r region; do
    ...
    run_or_print "terraform apply ${TERRAFORM_APPLY_ARGS} -target module.${cloud_provider}_base -var-file=../../terraform.tfvars.json -var=${cloud_provider}_k8s_region=${region} -var=cluster_name=${cluster_name} -var=cluster_id=${index}"
  ...
  print_info "Finished to deploy infra on cloud '${cloud_provider}'"
}
...
function destroy_infra() {
  ...
  while read -r region; do
    ...
    run_or_print "cd infra/${cloud_provider}"
    run_or_print "terraform workspace select ${cloud_provider}-${index}-${region}"
    run_or_print "terraform destroy ${TERRAFORM_DESTROY_ARGS} -var-file=../../terraform.tfvars.json -var=${cloud_provider}_k8s_region=${region} -var=cluster_id=${index} -var=cluster_name=${cluster_name}"
    run_or_print "terraform workspace select default"
  ...
  print_info "Finished to destroy infra on cloud '${cloud_provider}'"
}
